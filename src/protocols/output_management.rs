...
use naru_config::{FloatOrInt, OutputName, Vrr};
use naru_ipc::Transform;
...
pub struct OutputManagementManagerState {
    ...
    current_state: HashMap<OutputId, naru_ipc::Output>,
    current_config: naru_config::Outputs,
}
...
enum OutputConfigurationState {
    Ongoing(HashMap<OutputId, naru_config::Output>),
    Finished,
}
...
impl OutputManagementManagerState {
    ...
    pub fn on_config_changed(&mut self, new_config: naru_config::Outputs) {
        self.current_config = new_config;
    }

    pub fn notify_changes(&mut self, new_state: HashMap<OutputId, naru_ipc::Output>) {
                ...
                match new_config.entry(*output) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        let mut config = g_state
                            ...
                            .find(&name)
                            .cloned()
                            .unwrap_or_else(|| naru_config::Output {
    ...
    fn request(
        ...
        match request {
            zwlr_output_configuration_head_v1::Request::SetMode { mode } => {
                ...
                new_config.mode = Some(naru_config::output::Mode {
                    custom: false,
                    mode: naru_ipc::ConfiguredMode {
                        width: mode.width,
                        height: mode.height,
            ...
            zwlr_output_configuration_head_v1::Request::SetPosition { x, y } => {
                new_config.position = Some(naru_config::Position { x, y });
            }
