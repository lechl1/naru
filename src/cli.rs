...
use naru_ipc::{Action, OutputAction};
...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/naru/config.kdl`).
    ///
    /// This can also be set with the `NARU_CONFIG` environment variable. If both are set, the
...
#[derive(Subcommand)]
pub enum Sub {
    /// Communicate with the running naru instance.
    Msg {
        #[command(subcommand)]
        msg: Msg,
...
pub enum Msg {
    ...
    Output {
        /// Output name.
        ///
        /// Run `naru msg outputs` to see the output names.
        #[arg()]
        output: String,
    ...
    EventStream,
    /// Print the version of the running naru instance.
    Version,
    /// Request an error from the running naru instance.
    RequestError,
