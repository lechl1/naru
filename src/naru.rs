...
use naru_config::debug::PreviewRender;
use naru_config::{
...
#[cfg(feature = "dbus")]
use crate::dbus::freedesktop_locale1::Locale1ToNaru;
#[cfg(feature = "dbus")]
use crate::dbus::freedesktop_login1::Login1ToNaru;
#[cfg(feature = "dbus")]
use crate::dbus::gnome_shell_introspect::{self, IntrospectToNaru, NaruToIntrospect};
#[cfg(feature = "dbus")]
use crate::dbus::gnome_shell_screenshot::{NaruToScreenshot, ScreenshotToNaru};
...
pub struct Naru {
    ...
    pub config_file_output_config: naru_config::Outputs,
    ...
    ///
    /// This is `None` when creating `Naru` without a Wayland socket.
    ...
    // unredirected.
    //
    // https://github.com/lechl1/naru/issues/619
    #[cfg(test)]
    ...
    pub pick_color: Option<async_channel::Sender<Option<naru_ipc::PickedColor>>>,
...
// The surfaces here are always toplevel surfaces focused as far as naru's logic is concerned, even
...
pub struct PointContents {
    ...
    //
    // Can be `None` even when `window` is set, for example when the pointer is over the naru
...
impl CastTarget {
    ...
    pub fn matches(&self, ipc: &naru_ipc::CastTarget) -> bool {
        use CastTarget::*;
        match (self, ipc) {
            (Nothing, naru_ipc::CastTarget::Nothing {}) => true,
            (Output { name, .. }, naru_ipc::CastTarget::Output { name: ipc_name }) => {
                name == ipc_name
            }
            (Window { id }, naru_ipc::CastTarget::Window { id: ipc_id }) => id == ipc_id,
            _ => false,
        }
    }

    pub fn make_ipc(&self) -> naru_ipc::CastTarget {
        use CastTarget::*;
        match self {
            Nothing => naru_ipc::CastTarget::Nothing {},
            Output { name, .. } => naru_ipc::CastTarget::Output { name: name.clone() },
            Window { id } => naru_ipc::CastTarget::Window { id: *id },
        }
    }
}
...
impl State {
    pub fn new(
        ...
        backend.init(&mut naru);

        let mut state = Self { backend, naru };
    ...
    pub fn refresh_and_flush_clients(&mut self) {
        ...
        self.naru.redraw_queued_outputs(&mut self.backend);

        {
            let _span = tracy_client::span!("flush_clients");
            self.naru.display_handle.flush_clients().unwrap();
        }

        #[cfg(feature = "dbus")]
        self.naru.update_locked_hint();

        // Clear the time so it's fetched afresh next iteration.
        self.naru.clock.clear();
        self.naru.pointer_inactivity_timer_got_reset = false;
        self.naru.notified_activity_this_iteration = false;
    }
    ...
    pub fn set_lid_closed(&mut self, is_closed: bool) {
        if self.naru.is_lid_closed == is_closed {
            return;
        }

        debug!("laptop lid {}", if is_closed { "closed" } else { "opened" });
        self.naru.is_lid_closed = is_closed;
        self.backend.on_output_config_changed(&mut self.naru);
    }
    ...
    fn refresh(&mut self) {
        ...
        self.naru.refresh_window_states();

        // Needs to be called after updating the keyboard focus.
        self.naru.refresh_layout();

        self.naru.cursor_manager.check_cursor_image_surface_alive();
        self.naru.refresh_pointer_outputs();
        self.naru.global_space.refresh();
        self.naru.refresh_idle_inhibit();
        self.refresh_pointer_contents();
        foreign_toplevel::refresh(self);
        ext_workspace::refresh(self);

        #[cfg(feature = "xdp-gnome-screencast")]
        self.naru.refresh_mapped_cast_outputs();
        // Should happen before refresh_window_rules(), but after anything that can start or stop
        // screencasts.
        #[cfg(feature = "xdp-gnome-screencast")]
        self.naru.refresh_mapped_cast_window_rules();
        self.ipc_refresh_casts();

        self.naru.refresh_window_rules();
        ...
        // Needs to be called after updating the keyboard focus.
        #[cfg(feature = "dbus")]
        self.naru.refresh_a11y();
    }

    fn notify_blocker_cleared(&mut self) {
        let dh = self.naru.display_handle.clone();
        while let Ok(client) = self.naru.blocker_cleared_rx.try_recv() {
    ...
    pub fn move_cursor(&mut self, location: Point<f64, Logical>) {
        let mut under = match self.naru.pointer_visibility {
            PointerVisibility::Disabled => PointContents::default(),
            _ => self.naru.contents_under(location),
        };

        // Disable the hidden pointer if the contents underneath have changed.
        if !self.naru.pointer_visibility.is_visible() && self.naru.pointer_contents != under {
    ...
    pub fn move_cursor_to_focused_tile(&mut self, mode: CenterCoords) -> bool {
        if !self.naru.keyboard_focus.is_layout() {
            return false;
        }
        ...
        let Some(output) = self.naru.layout.active_output() else {
            return false;
        };
        let monitor = self.naru.layout.monitor_for_output(output).unwrap();
    ...
    pub fn focus_default_monitor(&mut self) {
        ...
        let Some(mut target) = self.naru.sorted_outputs.first().cloned() else {
        ...
        for config in &config.outputs.0 {
            ...
            if let Some(output) = self.naru.output_by_name_match(&config.name) {
                target = output.clone();
                break;
            }
        }
        ...
        self.naru.layout.focus_output(&target);
    ...
    pub fn focus_window(&mut self, window: &Window) {
        let active_output = self.naru.layout.active_output().cloned();

        self.naru.layout.activate_window(window);

        let new_active = self.naru.layout.active_output().cloned();
    ...
    pub fn confirm_mru(&mut self) {
        if let Some(window) = self.naru.close_mru(MruCloseRequest::Confirm) {
            self.focus_window(&window);
        }
    }

    pub fn maybe_warp_cursor_to_focus(&mut self) -> bool {
        let focused = match self.naru.config.borrow().input.warp_mouse_to_focus {
            None => return false,
    ...
    pub fn refresh_pointer_contents(&mut self) {
        let _span = tracy_client::span!("Naru::refresh_pointer_contents");
            ...
            && !self.naru.screenshot_ui.is_open()
        {
            ...
            if let Some((output, _)) = self.naru.output_under(location) {
                let monitor = self.naru.layout.monitor_for_output(output).unwrap();
    ...
    pub fn update_pointer_contents(&mut self) -> bool {
        let _span = tracy_client::span!("Naru::update_pointer_contents");
        ...
        if self.naru.pointer_contents == under {
            return false;
        }
    ...
    pub fn refresh_popup_grab(&mut self) {
        if let Some(grab) = &mut self.naru.popup_grab {
            if grab.grab.has_ended() {
                self.naru.popup_grab = None;
            }
        }
    }
    ...
    pub fn update_keyboard_focus(&mut self) {
        ...
        if let Some(surface) = &self.naru.layer_shell_on_demand_focus {
            // Still alive and has on-demand interactivity.
            let mut good = surface.alive()
                && surface.cached_state().keyboard_interactivity
                    == wlr_layer::KeyboardInteractivity::OnDemand;

            if let Some(mapped) = self.naru.mapped_layer_surfaces.get(surface) {
        ...
        let focus = if self.naru.exit_confirm_dialog.is_open() {
            ...
            let layer_grab = self.naru.popup_grab.as_ref().and_then(|g| {
                layers
            ...
            let excl_focus_on_layer = |layer| {
                layers.layers_on(layer).find_map(|surface| {
                    ...
                    let mapped = self.naru.mapped_layer_surfaces.get(surface)?;
                    if mapped.place_within_backdrop() {
                        return None;
                    }
            ...
            let on_d_focus_on_layer = |layer| {
                layers.layers_on(layer).find_map(|surface| {
                    let is_on_demand_surface =
                        Some(surface) == self.naru.layer_shell_on_demand_focus.as_ref();
        ...
        if self.naru.keyboard_focus != focus {
            trace!(
                "keyboard focus changed from {:?} to {:?}",
                self.naru.keyboard_focus,
                focus
            );
            ...
            } = &focus
            {
                if let Some((mapped, _)) = self.naru.layout.find_window_and_output_mut(surface) {
                    ...
                    let debounce = self.naru.config.borrow().recent_windows.debounce_ms;
                    ...
                    if mapped.get_focus_timestamp().is_none() || debounce.is_zero() {
                        ...
                        let focus_token = self
                            ...
                            .insert_source(timer, move |_, _, state| {
                                state.naru.mru_apply_keyboard_commit();
                                TimeoutAction::Drop
                            })
                            .unwrap();
                        if let Some(PendingMruCommit { token, .. }) =
                            self.naru.pending_mru_commit.replace(PendingMruCommit {
                                id: mapped.id(),
                                token: focus_token,
                                stamp,
                            })
            ...
            if let Some(grab) = self.naru.popup_grab.as_mut() {
                if grab.has_keyboard_grab && Some(&grab.root) != focus.surface() {
                    ...
                    self.naru.seat.get_pointer().unwrap().unset_grab(
                        self,
            ...
            if self.naru.config.borrow().input.keyboard.track_layout == TrackLayout::Window {
                ...
                if let Some(current_focus) = self.naru.keyboard_focus.surface() {
            ...
            self.naru.keyboard_focus.clone_from(&focus);
    ...
    fn load_xkb_file(&mut self) {
        let xkb_file = self.naru.config.borrow().input.keyboard.xkb.file.clone();
    ...
    pub fn reload_config(&mut self, config: Result<Config, ()>) {
        ...
        let mut config = match config {
            Ok(config) => config,
            Err(()) => {
                self.naru.config_error_notification.show();
                self.naru.queue_redraw_all();

                #[cfg(feature = "dbus")]
                self.naru.a11y_announce_config_error();

                return;
            }
        };

        self.naru.config_error_notification.hide();

        // Find & orphan removed named workspaces.
        let mut removed_workspaces: Vec<String> = vec![];
        for ws in &self.naru.config.borrow().workspaces {
            if !config.workspaces.iter().any(|w| w.name == ws.name) {
                removed_workspaces.push(ws.name.0.clone());
            }
        }
        for name in removed_workspaces {
            self.naru.layout.unname_workspace(&name);
        }

        self.naru.layout.update_config(&config);
        for mapped in self.naru.mapped_layer_surfaces.values_mut() {
            mapped.update_config(&config);
        }

        // Create new named workspaces.
        for ws_config in &config.workspaces {
            self.naru.layout.ensure_named_workspace(ws_config);
        }

        let rate = 1.0 / config.animations.slowdown.max(0.001);
        self.naru.clock.set_rate(rate);
        self.naru
            .clock
        ...
        *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
        ...
        let mut old_config = self.naru.config.borrow_mut();
        ...
        if config.cursor != old_config.cursor {
            ...
            self.naru.cursor_texture_cache.clear();
        }
        ...
        if config.outputs != self.naru.config_file_output_config {
        ...
        if new_mod_key != self.backend.mod_key(&old_config) || binds_changed {
            ...
            self.naru.mods_with_mouse_binds = mods_with_mouse_binds(new_mod_key, &config.binds);
            self.naru.mods_with_wheel_binds = mods_with_wheel_binds(new_mod_key, &config.binds);
            self.naru.mods_with_finger_scroll_binds =
        ...
        *old_config = config;
        ...
        if let Some(mut xkb) = reload_xkb {
            ...
            if set_xkb_config {
                // If xkb is unset in the naru config, use settings from locale1.
                if xkb == Xkb::default() {
                    trace!("using xkb from locale1");
                    xkb = self.naru.xkb_from_locale1.clone().unwrap_or_default();
                }
        ...
        if libinput_config_changed {
            let config = self.naru.config.borrow();
            for mut device in self.naru.devices.iter().cloned() {
                apply_libinput_settings(&config.input, &mut device);
            }
        }

        if ignored_nodes_changed {
            self.backend.update_ignored_nodes_config(&mut self.naru);
        }
        ...
        if window_rules_changed {
            self.naru.recompute_window_rules();
        }

        if layer_rules_changed {
            self.naru.recompute_layer_rules();
        }

        if shaders_changed {
            self.naru.update_shaders();
        }
        ...
        if xwls_changed {
            ...
            let display_name = (!config.xwayland_satellite.off)
                .then_some(self.naru.satellite.as_ref())
                .flatten()
    ...
    pub fn reload_output_config(&mut self) {
        ...
        for output in self.naru.global_space.outputs() {
            let name = output.user_data().get::<OutputName>().unwrap();
            let full_config = self.naru.config.borrow_mut();
                ...
                || output.current_transform() != transform
            {
                ...
                self.naru.ipc_outputs_changed = true;
            ...
            if let Some(state) = self.naru.output_state.get_mut(output) {
            ...
            for mon in self.naru.layout.monitors_mut() {
        ...
        self.naru.reposition_outputs(None);

        if let Some(touch) = self.naru.seat.get_touch() {
            touch.cancel(self);
        }

        let config = self.naru.config.borrow().outputs.clone();
        self.naru.output_management_state.on_config_changed(config);
    }

    pub fn modify_output_config<F>(&mut self, name: &str, fun: F)
    where
        F: FnOnce(&mut naru_config::Output),
    {
        ...
        let temp;
        let match_name = if let Some(output) = self.naru.output_by_name_match(name) {
        ...
        let mut config = self.naru.config.borrow_mut();
        let config = if let Some(config) = config.outputs.find_mut(match_name) {
            config
        } else {
            config.outputs.0.push(naru_config::Output {
    ...
    pub fn apply_transient_output_config(&mut self, name: &str, action: naru_ipc::OutputAction) {
        self.modify_output_config(name, move |config| match action {
            naru_ipc::OutputAction::Off => config.off = true,
            naru_ipc::OutputAction::On => config.off = false,
            naru_ipc::OutputAction::Mode { mode } => {
                config.mode = match mode {
                    naru_ipc::ModeToSet::Automatic => None,
                    naru_ipc::ModeToSet::Specific(mode) => Some(naru_config::output::Mode {
                        custom: false,
                        mode,
                    }),
                };
            ...
            naru_ipc::OutputAction::CustomMode { mode } => {
                config.mode = Some(naru_config::output::Mode { custom: true, mode });
                config.modeline = None;
            }
            naru_ipc::OutputAction::Modeline {
                clock,
                hdisplay,
                hsync_start,
                hsync_end,
                ...
                config.modeline = Some(naru_config::output::Modeline {
                    clock,
                    hdisplay,
                    hsync_start,
                    hsync_end,
            ...
            naru_ipc::OutputAction::Scale { scale } => {
                config.scale = match scale {
                    naru_ipc::ScaleToSet::Automatic => None,
                    naru_ipc::ScaleToSet::Specific(scale) => Some(FloatOrInt(scale)),
                }
            }
            naru_ipc::OutputAction::Transform { transform } => config.transform = transform,
            naru_ipc::OutputAction::Position { position } => {
                config.position = match position {
                    naru_ipc::PositionToSet::Automatic => None,
                    naru_ipc::PositionToSet::Specific(position) => Some(naru_config::Position {
                        x: position.x,
                        y: position.y,
                    }),
                }
            }
            naru_ipc::OutputAction::Vrr { vrr } => {
                config.variable_refresh_rate = if vrr.vrr {
                    Some(naru_config::Vrr {
    ...
    pub fn refresh_ipc_outputs(&mut self) {
        if !self.naru.ipc_outputs_changed {
            return;
        }
        self.naru.ipc_outputs_changed = false;
        ...
        #[cfg(feature = "dbus")]
        self.naru.on_ipc_outputs_changed();

        let new_config = self.backend.ipc_outputs().lock().unwrap().clone();
        self.naru.output_management_state.notify_changes(new_config);
    }

    pub fn open_screenshot_ui(&mut self, show_pointer: bool, path: Option<String>) {
        if self.naru.is_locked() || self.naru.screenshot_ui.is_open() {
            return;
        }
        ...
        let default_output = self
            .naru
            .output_under_cursor()
            .or_else(|| self.naru.layout.active_output().cloned());
        let Some(default_output) = default_output else {
            return;
        };

        self.naru.update_render_elements(None);

        let Some(screenshots) = self
            .backend
            .with_primary_renderer(|renderer| self.naru.capture_screenshots(renderer).collect())
        else {
            return;
        };
    ...
    pub fn handle_pick_color(&mut self, tx: async_channel::Sender<Option<naru_ipc::PickedColor>>) {
        let pointer = self.naru.seat.get_pointer().unwrap();
    ...
    pub fn confirm_screenshot(&mut self, write_to_disk: bool) {
        let ScreenshotUi::Open { path, .. } = &mut self.naru.screenshot_ui else {
            return;
        };
        let path = path.take();

        self.backend.with_primary_renderer(|renderer| {
            match self.naru.screenshot_ui.capture(renderer) {
                Ok((size, pixels)) => {
                    if let Err(err) = self.naru.save_screenshot(size, pixels, write_to_disk, path) {
    ...
    pub fn store_unmap_snapshot(&mut self, window: &Window, output: Option<&Output>) {
        ...
        self.naru.update_xray_render_elements(output);
        ...
        self.backend.with_primary_renderer(|renderer| {
            if let Some(output) = output {
                ...
                self.naru.fill_xray_elements(ctx.r(), output);
                ...
                let has_blocked_out = self.naru.has_blocked_out_background_layers(output);
                if has_blocked_out {
                    let screencast_ctx = RenderCtx {
                        target: RenderTarget::Screencast,
                        ..ctx.r()
                    };
                    self.naru.fill_xray_elements(screencast_ctx, output);
                }

                let state = self.naru.output_state.get_mut(output).unwrap();
                self.naru.layout.store_unmap_snapshot(
                    renderer,
                    Some(&mut state.xray),
                    has_blocked_out,
                    window,
                ...
                self.naru.clear_xray_elements(output);
            } else {
                self.naru
                    .layout
    ...
    pub fn on_screen_shot_msg(
        &mut self,
        to_screenshot: &async_channel::Sender<NaruToScreenshot>,
        msg: ScreenshotToNaru,
    ) {
        match msg {
            ScreenshotToNaru::TakeScreenshot { include_cursor } => {
                self.handle_take_screenshot(to_screenshot, include_cursor);
            }
            ScreenshotToNaru::PickColor(tx) => {
    ...
    fn handle_take_screenshot(
        ...
        let rv = self.backend.with_primary_renderer(|renderer| {
            let on_done = {
                ...
                move |path| {
                    let msg = NaruToScreenshot::ScreenshotResult(Some(path));
            ...
            if let Err(err) = res {
                ...
                let msg = NaruToScreenshot::ScreenshotResult(None);
                if let Err(err) = to_screenshot.send_blocking(msg) {
    ...
    pub fn on_introspect_msg(
        &mut self,
        to_introspect: &async_channel::Sender<NaruToIntrospect>,
        msg: IntrospectToNaru,
    ) {
        use crate::utils::with_toplevel_role;

        let IntrospectToNaru::GetWindows = msg;
        ...
        #[cfg(feature = "xdp-gnome-screencast")]
        windows.insert(
            self.naru.casting.dynamic_cast_id_for_portal.get(),
            gnome_shell_introspect::WindowProperties {
                title: String::from("naru Dynamic Cast Target"),
                app_id: String::from("rs.bxt.naru.desktop"),
            },
        );

        self.naru.layout.with_windows(|mapped, _, _, _| {
        ...
        let msg = NaruToIntrospect::Windows(windows);
    ...
    #[cfg(feature = "dbus")]
    pub fn on_login1_msg(&mut self, msg: Login1ToNaru) {
        let Login1ToNaru::LidClosedChanged(is_closed) = msg;
    ...
    #[cfg(feature = "dbus")]
    pub fn on_locale1_msg(&mut self, msg: Locale1ToNaru) {
        let Locale1ToNaru::XkbChanged(xkb) = msg;

        trace!("locale1 xkb settings changed: {xkb:?}");
        let xkb = self.naru.xkb_from_locale1.insert(xkb);

        {
            let config = self.naru.config.borrow();
            if config.input.keyboard.xkb != Xkb::default() {
                trace!("ignoring locale1 xkb change because naru config has xkb settings");
                return;
            }
        }
...
impl Naru {
    ...
    pub fn new(
        ...
        let _span = tracy_client::span!("Naru::new");
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    state.naru.activation_state.retain_tokens(|_, token_data| {
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    state.naru.send_frame_callbacks_on_fallback_timer();
        ...
        let socket_name = create_wayland_socket.then(|| {
            ...
            event_loop
                .insert_source(socket_source, move |client, _, state| {
                    state.naru.insert_client(NewClient {
                        client,
                        restricted: false,
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    ...
                    state.naru.is_at_startup = false;
                    state.naru.recompute_window_rules();
                    state.naru.recompute_layer_rules();
                    TimeoutAction::Drop
                },
            )
            .unwrap();
        ...
        naru.reset_pointer_inactivity_timer();
    ...
    pub fn inhibit_power_key(&mut self) -> anyhow::Result<()> {
        ...
        let message = conn.call_method(
            ...
            "Inhibit",
            &("handle-power-key", "naru", "Power key handling", "block"),
        )?;
    ...
    pub fn reposition_outputs(&mut self, new_output: Option<&Output>) {
        let _span = tracy_client::span!("Naru::reposition_outputs");
        ...
        struct Data {
            ...
            config: Option<naru_config::Position>,
        }
    ...
    /// exists in naru. Right after the output global is disabled, but before it is removed for
    /// good, `Output::from_resource` will succeed, but since naru already forgot the output,
    ...
    pub fn redraw_queued_outputs(&mut self, backend: &mut Backend) {
        let _span = tracy_client::span!("Naru::redraw_queued_outputs");
    ...
    pub fn render_pointer<R: NaruRenderer>(
        &self,
        renderer: &mut R,
        output: &Output,
        push: &mut dyn FnMut(PointerRenderElements<R>),
    ) {
        let _span = tracy_client::span!("Naru::render_pointer");
    ...
    pub fn refresh_pointer_outputs(&mut self) {
        ...
        let _span = tracy_client::span!("Naru::refresh_pointer_outputs");
    ...
    pub fn refresh_idle_inhibit(&mut self) {
        let _span = tracy_client::span!("Naru::refresh_idle_inhibit");
    ...
    pub fn refresh_window_states(&mut self) {
        let _span = tracy_client::span!("Naru::refresh_window_states");
    ...
    pub fn refresh_window_rules(&mut self) {
        let _span = tracy_client::span!("Naru::refresh_window_rules");
    ...
    pub fn advance_animations(&mut self) {
        let _span = tracy_client::span!("Naru::advance_animations");
    ...
    pub fn render_to_vec<R: NaruRenderer>(
        &self,
        ctx: RenderCtx<R>,
        output: &Output,
    ...
    pub fn render<R: NaruRenderer>(
        ...
        let _span = tracy_client::span!("Naru::render");
    ...
    pub fn fill_xray_elements(&self, mut ctx: RenderCtx<GlesRenderer>, output: &Output) {
        let _span = tracy_client::span!("Naru::fill_xray_elements");
    ...
    fn render_layer_normal<R: NaruRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        ns: Option<usize>,
        layer_map: &LayerMap,
    ...
    #[allow(clippy::too_many_arguments)]
    fn render_layer_popups<R: NaruRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        ns: Option<usize>,
        layer_map: &LayerMap,
    ...
    fn redraw(&mut self, backend: &mut Backend, output: &Output) {
        let _span = tracy_client::span!("Naru::redraw");
    ...
    pub fn refresh_on_demand_vrr(&mut self, backend: &mut Backend, output: &Output) {
        let _span = tracy_client::span!("Naru::refresh_on_demand_vrr");
    ...
    pub fn update_primary_scanout_output(
        ...
        // as in naru they can only be rendered on a single output at a time.
        //
    ...
    pub fn send_dmabuf_feedbacks(
        ...
        let _span = tracy_client::span!("Naru::send_dmabuf_feedbacks");
    ...
    pub fn send_frame_callbacks(&mut self, output: &Output) {
        let _span = tracy_client::span!("Naru::send_frame_callbacks");
    ...
    pub fn send_frame_callbacks_on_fallback_timer(&mut self) {
        let _span = tracy_client::span!("Naru::send_frame_callbacks_on_fallback_timer");
    ...
    pub fn render_for_screencopy_with_damage(
        ...
        let _span = tracy_client::span!("Naru::render_for_screencopy_with_damage");
    ...
    pub fn render_for_screencopy_without_damage(
        ...
        let _span = tracy_client::span!("Naru::render_for_screencopy");
    ...
    pub fn screenshot(
        ...
        let _span = tracy_client::span!("Naru::screenshot");
    ...
    pub fn screenshot_window(
        ...
        let _span = tracy_client::span!("Naru::screenshot_window");
    ...
    pub fn screenshot_all_outputs(
        ...
        let _span = tracy_client::span!("Naru::screenshot_all_outputs");
        ...
        if self.output_state.is_empty() {
            ...
            let deadline_token = self
                ...
                .insert_source(timer, |_, _, state| {
                    ...
                    state.naru.continue_to_locking();
                    TimeoutAction::Drop
                })
                .unwrap();
    ...
    pub fn on_ipc_outputs_changed(&self) {
        let _span = tracy_client::span!("Naru::on_ipc_outputs_changed");
    ...
    pub fn do_screen_transition(&mut self, renderer: &mut GlesRenderer, delay_ms: Option<u16>) {
        let _span = tracy_client::span!("Naru::do_screen_transition");
    ...
    pub fn recompute_window_rules(&mut self) {
        let _span = tracy_client::span!("Naru::recompute_window_rules");
    ...
    pub fn recompute_layer_rules(&mut self) {
        let _span = tracy_client::span!("Naru::recompute_layer_rules");
    ...
    pub fn reset_pointer_inactivity_timer(&mut self) {
        ...
        let _span = tracy_client::span!("Naru::reset_pointer_inactivity_timer");
        ...
        let token = self
            .event_loop
            .insert_source(timer, move |_, _, state| {
                state.naru.pointer_inactivity_timer = None;

                // If the pointer is already invisible, don't reset it back to Hidden causing one
                // frame of hover.
                if state.naru.pointer_visibility.is_visible() {
                    state.naru.pointer_visibility = PointerVisibility::Hidden;
    ...
    pub fn notify_activity(&mut self) {
        ...
        let _span = tracy_client::span!("Naru::notify_activity");
