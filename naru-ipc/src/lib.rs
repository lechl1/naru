//! Types for communicating with naru via IPC.
//!
//! After connecting to the naru socket, you can send [`Request`]s. Naru will process them one by
//! one, in order, and to each request it will respond with a single [`Reply`], which is a `Result`
//! wrapping a [`Response`].
//!
//! If you send a [`Request::EventStream`], naru will *stop* reading subsequent [`Request`]s, and
...
//!
//! 1. Read the socket filesystem path from [`socket::SOCKET_PATH_ENV`] (`$NARU_SOCKET`).
...
//! 3. Naru will respond with a single line JSON-formatted [`Reply`].
//! 4. You can keep writing [`Request`]s, each on a single line, and read [`Reply`]s, also each on a
//!    separate line.
//! 5. After you request an event stream, naru will keep responding with JSON-formatted [`Event`]s,
//!    on a single line each.
//!
//! ## Backwards compatibility
//!
//! This crate follows the naru version. It is **not** API-stable in terms of the Rust semver. In
...
//!   the types.
//! - `clap`: derives the clap CLI parsing traits for some types. Used internally by naru itself.
#![warn(missing_docs)]
...
/// Request from client to naru.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Request {
    /// Request the version string for the running naru instance.
    Version,
...
/// Successful response from naru to client.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Response {
    /// A request that does not need a response was handled successfully.
    Handled,
    /// The version string for the running naru instance.
    Version(String),
...
/// Actions that naru can perform.
// Variants in this enum should match the spelling of the ones in naru-config. Most, but not all,
// variants from naru-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
pub enum Action {
    ...
    /// naru's config file watcher to notice the changes.
    LoadConfigFile {
...
pub enum OutputAction {
    ...
    Mode {
        ...
        ///
        /// Run `naru msg outputs` to see the available modes.
        #[cfg_attr(feature = "clap", arg())]
        mode: ModeToSet,
    },
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ModeToSet {
    /// Naru will pick the mode automatically.
    Automatic,
    /// Specific mode.
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ScaleToSet {
    /// Naru will pick the scale automatically.
    Automatic,
    /// Specific scale.
    Specific(f64),
}
...
/// some optional properties are present or absent for certain window types may change across naru
/// releases.
///
...
pub struct WindowLayout {
    ...
    ///
    /// Does not include naru decorations like borders.
    ///
    ...
    ///
    /// This is the same "workspace view" as in gradients' `relative-to` in the naru config.
...
pub struct Workspace {
    ...
    ///
    /// This is the same index you can use for requests like `naru msg action focus-workspace`.
    ///
