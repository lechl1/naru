//! Helper for blocking communication over the naru socket.
...
/// Name of the environment variable containing the naru IPC socket path.
pub const SOCKET_PATH_ENV: &str = "NARU_SOCKET";

/// Helper for blocking communication over the naru socket.
///
/// This struct is used to communicate with the naru IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default naru IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within naru?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the naru IPC socket at the given path.
    ...
    /// Sends a request to naru and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from naru
    /// * `Ok(Err(message))`: error message from naru
    /// * `Err(error)`: error communicating with naru
