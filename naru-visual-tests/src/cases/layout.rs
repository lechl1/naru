...
use naru::animation::Clock;
use naru::layout::{ActivateWindow, AddWindowTarget, LayoutElement as _, Options, SizingMode};
use naru::render_helpers::{RenderCtx, RenderTarget};
use naru_config::{Color, OutputName, PresetSize};
...
pub struct Layout {
    ...
    clock: Clock,
    layout: naru::layout::Layout<TestWindow>,
    start_time: Duration,
    ...
    pub fn new(args: Args) -> Self {
        ...
        let options = Options {
            layout: naru_config::Layout {
                focus_ring: naru_config::FocusRing {
                    off: true,
                    ..Default::default()
                },
        ...
        let mut layout = naru::layout::Layout::with_options(clock.clone(), options);
