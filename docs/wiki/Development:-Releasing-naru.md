This is a checklist of things to release a new naru version.
...
- Dependency example in `naru-ipc/README.md`
- Dependency example in `naru-ipc/src/lib.rs`
...
- Run `cargo package -p naru-ipc` and make sure it succeeds.
- Make sure the CI passes.
- Make sure the naru-git COPR build passes.
...
- build and test naru with that dependency archive,
...
## Update the naru COPR spec, update licenses in .spec.rpkg

You can grab the previous spec from [the last build](https://copr.fedorainfracloud.org/coprs/yalter/naru/builds/) in the COPR.
...
Update it in both the COPR spec and in `naru.spec.rpkg` accordingly.

If you had to update `naru.spec.rpkg` and therefore make another commit to the naru repo, make sure to update the commit hash in the COPR spec again.
...
## Publish the naru-ipc crate

```
cargo publish -p naru-ipc
```
...
- Generate data:
    ...
    ```
    wlprobe > ./src/data/compositors/naru.json
    ```
