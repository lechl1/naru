You can communicate with the running naru instance over an IPC socket.
Check `naru msg --help` for available commands.

The `--json` flag prints the response in JSON, rather than formatted.
For example, `naru msg --json outputs`.

> [!TIP]
> If you're getting parsing errors from `naru msg` after upgrading naru, make sure that you've restarted naru itself.
> You might be trying to run a newer `naru msg` against an older `naru` compositor.

### Event Stream

<sup>Since: 0.1.9</sup>

While most naru IPC requests return a single response, the event stream request will make naru continuously stream events into the IPC connection until it is closed.
...
This way, your state can never "desync" from naru, and you don't need to make any other IPC information requests.
...
To get a taste of the events, run `naru msg event-stream`.
Though, this is more of a debug function than anything.
You can get raw events from `naru msg --json event-stream`, or by connecting to the naru socket and requesting an event stream manually.

You can find the full list of events along with documentation [here](https://lechl1.github.io/naru/naru_ipc/enum.Event.html).

### Programmatic Access

`naru msg --json` is a thin wrapper over writing and reading to a socket.
When implementing more complex scripts and modules, you're encouraged to access the socket directly.

Connect to the UNIX domain socket located at `$NARU_SOCKET` in the filesystem.
...
You can use `socat` to test communicating with naru directly:

```sh
$ socat STDIO "$NARU_SOCKET"
"FocusedWindow"
{"Ok":{"FocusedWindow":{"id":12,"title":"t socat STDIO /run/u ~","app_id":"Alacritty","workspace_id":6,"is_focused":true}}}
```

The reply is an `Ok` or an `Err` wrapping the same JSON object as you get from `naru msg --json`.

For more complex requests, you can use `socat` to find how `naru msg` formats them:

```sh
$ socat STDIO UNIX-LISTEN:temp.sock
# then, in a different terminal:
$ env NARU_SOCKET=./temp.sock naru msg action focus-workspace 2
# then, look in the socat terminal:
{"Action":{"FocusWorkspace":{"reference":{"Index":2}}}}
```

You can find all available requests and response types in the [naru-ipc sub-crate documentation](https://lechl1.github.io/naru/naru_ipc/).
...
The `naru-ipc` sub-crate (like other naru sub-crates) is *not* API-stable in terms of the Rust semver; rather, it follows the version of naru itself.
