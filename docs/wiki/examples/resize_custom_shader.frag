...
vec4 stretch_or_crop_next(vec3 coords_curr_geo, vec3 size_curr_geo) {
    vec3 coords_next_geo = naru_curr_geo_to_next_geo * coords_curr_geo;

    vec3 coords_stretch = naru_geo_to_tex_next * coords_curr_geo;
    vec3 coords_crop = naru_geo_to_tex_next * coords_next_geo;

    // We can crop if the current window size is smaller than the next window
    // size. One way to tell is by comparing to 1.0 the X and Y scaling
    // coefficients in the current-to-next transformation matrix.
    bool can_crop_by_x = naru_curr_geo_to_next_geo[0][0] <= 1.0;
    bool can_crop_by_y = naru_curr_geo_to_next_geo[1][1] <= 1.0;
    ...
    vec4 color = texture2D(naru_tex_next, coords.st);
...
vec4 crossfade_or_crop_next(vec3 coords_curr_geo, vec3 size_curr_geo) {
    vec3 coords_next_geo = naru_curr_geo_to_next_geo * coords_curr_geo;
    vec3 coords_prev_geo = naru_curr_geo_to_prev_geo * coords_curr_geo;

    vec3 coords_crop = naru_geo_to_tex_next * coords_next_geo;
    vec3 coords_stretch = naru_geo_to_tex_next * coords_curr_geo;
    vec3 coords_stretch_prev = naru_geo_to_tex_prev * coords_curr_geo;
    ...
    if (crop) {
        ...
        if (coords_curr_geo.x < 0.0 || 1.0 < coords_curr_geo.x ||
            ...
            color = texture2D(naru_tex_next, coords_crop.st);
        }
    } else {
        // If we can't crop, then crossfade.
        color = texture2D(naru_tex_next, coords_stretch.st);
        vec4 color_prev = texture2D(naru_tex_prev, coords_stretch_prev.st);
        color = mix(color_prev, color, naru_clamped_progress);
    }
